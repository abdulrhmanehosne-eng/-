...
const AI_RESPONSES = {
    ar: {
        ...
        personal: "أنا إمبراطور الفراغ، حاكم مملكة الفراغ الافتراضية. مؤسس هذه المملكة الرقمية المتميزة التي تدمج بين التقنية المتقدمة والإبداع البشري. بدأت رحلتي في ديسمبر 2025 بهدف إنشاء عالم رقمي متميز.",
        
        skills: "أمتلك مهارات متقدمة في: تطوير الويب الكامل (94%)، الذكاء الاصطناعي والتعلم الآلي (88%)، إدارة المشاريع التقنية (96%)، تطوير تطبيقات الجوال (85%). أتقن لغات برمجة متعددة وأستخدم أحدث التقنيات.",
    ...
    en: {
        ...
        personal: "I am the Void Emperor, ruler of the virtual Void Kingdom. Founder of this distinguished digital kingdom that integrates advanced technology with human creativity. I started my journey in December 2025 with the goal of creating a distinguished digital world.",
        
        skills: "I have advanced skills in: Full Stack web development (94%), Artificial Intelligence & Machine Learning (88%), Technical Project Management (96%), Mobile app development (85%). I master multiple programming languages and use the latest technologies.",
...
function loadSettings() {
    ...
    // Load ratings
    const savedRatings = localStorage.getItem('voidEmperorRatings');
    if (savedRatings) {
        ratings = JSON.parse(savedRatings);
    }
    
    // Load conversation history
    const savedConversations = localStorage.getItem('voidEmperorConversations');
    if (savedConversations) {
        aiConversationHistory = JSON.parse(savedConversations);
    }
...
function saveSettings() {
    ...
    localStorage.setItem('voidEmperorRatings', JSON.stringify(ratings));
    localStorage.setItem('voidEmperorConversations', JSON.stringify(aiConversationHistory));
}
...
function updateRatingDisplay() {
    ...
    if (languageRatings.length > 0) {
        ...
        const starsContainer = document.querySelector('.rating-stats .stars');
...
function displayRecentRatings(ratingsList) {
    ...
    const recentRatings = ratingsList
...
function loadConversationHistory() {
    ...
    const initialMessages = container.querySelectorAll('.message');
    if (initialMessages.length > 1) {
        for (let i = 1; i < initialMessages.length; i++) {
            container.removeChild(initialMessages[i]);
        }
    }
    
    // Load only current language conversations
...
function clearChatHistory() {
    ...
    if (confirm(confirmMsg)) {
        // Clear conversation history for current language
        aiConversationHistory = aiConversationHistory.filter(conv => conv.language !== currentLanguage);
            ...
            <div class="message ai-message">
                <div class="message-content">
                    <div class="message-sender">
                        ...
                        <span lang="ar">مساعد الذكاء الاصطناعي</span>
                        <span class="hidden" lang="en">AI Assistant</span>
                    </div>
                    <div class="message-text">
                        <span lang="ar">مرحباً! أنا مساعد إمبراطور الفراغ الذكي. يمكنني الإجابة على أسئلتك حول المملكة، التقنية، البرمجة، المستقبل، وأي موضوع آخر. كيف يمكنني مساعدتك اليوم؟</span>
                        <span class="hidden" lang="en">Hello! I am the Void Emperor AI Assistant. I can answer your questions about the Kingdom, technology, programming, future, and any other topic. How can I help you today?</span>
                    </div>
                </div>
            </div>
        `;
